    ...
    for (const t of tiers) {
      ...
      try {
        await dispatchToChannels("signal", today, {
          ...
          text: `[예측·TOP10 ${t.tierKo}] ${label}\n${guide}\n무응답=현행 유지\n----\n${cur !== "none" ? `${t.out.reason.split(" — ")[0]} · ` : ""}강도 ${Math.round(t.out.confidence * 100)}% · 모니터링 60일 채점 중(실투자 판정은 기존 삼전·하닉 문자)${stopLine}${stateLine}`,
          smsSubject: "예측 TOP10",
        });
      } catch { /* 발송 실패 무시 */ }
    }
          ...
          if (minuteOfDay >= t5 + 1) {
            ...
            if (confBar && bar5 && hhmmToMin(bar5.time) >= t5 - 1) {
              ...
              await dispatchToChannels("signal", today, {
                ...
                // 상단=액션만·하단=부연 (사용자 지시 2026-08-01 2차)
                text: ok
                  ? `[예측·TOP10 본피셔 진행확인]\n▶유지 (비중 변경 없음)\n----\n${dirKo} 판정(${confT} ${confBar.close.toLocaleString()}원) 후 5분 — ${dirKo} 방향으로 ${Math.round(prog).toLocaleString()}원(${pctS(prog)}%) 전진 → 기준(전진 ${Math.round(need).toLocaleString()}원=10일평균폭의 10%) 충족, 정상. 과거 이 경우 29건: 평균 +1.21%·승률 62%·컷률 17%.`
                  : `[예측·TOP10 본피셔 진행경보]\n▶비중 축소 검토\n무응답=유지\n----\n${dirKo} 판정(${confT} ${confBar.close.toLocaleString()}원) 후 5분 — ${prog < 0 ? `판정 방향 반대로 ${Math.round(-prog).toLocaleString()}원(${pctS(-prog)}%) 역행` : `전진 ${Math.round(prog).toLocaleString()}원(${pctS(prog)}%)뿐`} → 기준(전진 ${Math.round(need).toLocaleString()}원=10일평균폭의 10%) 미달. 과거 이 경우 151건: 평균 +0.29%·컷률 27%.`,
