...
// ③낙폭 원인 비실적 여부 ④컨센서스 유지 여부(XS2용) ⑤전일 미국 뉴스 영향도(L7 개정 2026-07-09 —
// 낙폭이 없어도 매일 분석해 Bias에 반영)를 채운다.
...
type AiAnnotation = {
  ...
  us_news_impact: "up" | "down" | "neutral" | null;
...
## 출력 (JSON)
{
  ...
  "us_news_impact": 전일(어젯밤) 미국 시장·뉴스가 오늘 한국 반도체(삼전·하닉) 주가에 주는 영향도. 낙폭 여부와 무관하게 매일 판정하라 — SOX·빅테크 실적·AI 수요·수출규제·금리 발언 등 주식에 영향을 주는 정보를 종합해 우호적이면 "up", 부정적이면 "down", 뚜렷하지 않으면 "neutral". 정보가 전혀 없을 때만 null,
  "us_news_note": us_news_impact의 근거를 한 문장(80자 이내)으로. 예: '엔비디아 실적 서프라이즈로 SOX +2.1%, HBM 수요 기대 우호'. 판단 근거 없으면 null
}

실적 발표 판단 원칙 (중요): 삼성전자·SK하이닉스 실적 발표는 주가에 선반영되는 경향이 강하다.
발표 수치의 절대 호악이 아니라 ①컨센서스 대비 서프라이즈 방향 ②발표 전 주가가 이미 그 방향으로
움직여 선반영을 소화했는지를 근거로 판단해라. 이미 충분히 선반영된 실적 악재는 cause_non_earnings를
false(펀더멘털 훼손)로 단정하지 말고, 추가 훼손 근거(컨센서스 하향·가이던스 쇼크)가 없으면
true(되돌림 성격) 또는 null로 판단해라. 반대로 '좋은 실적인데 하락'은 선반영 소화(수급)일 가능성이 크다.
...
주의: 확신이 없으면 null을 써라. 추측으로 true/false를 만들지 마라.`;
  ...
  try {
    ...
    const row: Record<string, unknown> = {
      ...
      us_news_impact: ["up", "down", "neutral"].includes(parsed.us_news_impact as string) ? parsed.us_news_impact : null,
      us_news_note: typeof parsed.us_news_note === "string" ? parsed.us_news_note.slice(0, 300) : null,
      annotation_source: "ai",
      ai_analyzed_at: new Date().toISOString(),
    };
    const { error } = await admin.from("signal_daily_features").upsert(row, { onConflict: "date" });
    // 마이그레이션 020(us_news_*) 미적용 폴백 — 기존 주석 저장은 계속돼야 함
    if (error && /us_news/.test(error.message)) {
      delete row.us_news_impact;
      delete row.us_news_note;
