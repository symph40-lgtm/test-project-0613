...
//
// 하닉이 직전 추세(반전 창 시작 이전 30분 순변화 ≥ minTrendPct) 중일 때,
// 반대 방향으로 아래 중 하나가 성립하면 반전으로 판정한다:
//  1) 1분봉 1개 ≥ 0.8%  2) 1분봉 3개 합 ≥ 1.0%  3) 5분봉 1개 ≥ 1.0%
//  4) 5분봉 3개 합 ≥ 2.2%  5) 5분봉 5개 합 ≥ 2.7%  6) 7개 합 ≥ 3.2%
// 상승 반전 = 레버리지 검토 / 하락 반전 = 인버스 검토 (즉시 문자 — alerts.ts).
//
...
export type ReversalHit = {
  ...
  preMovePct: number;     // 직전 추세 크기 (%p, 반대 부호)
};
...
export function detectReversal(ticks: IntradayTick[]): ReversalHit | null {
  ...
  for (const c of conds) {
    ...
    // '추세 중의 반전'만 — 직전 추세가 반대 방향으로 충분히 진행돼 있어야 함
    ...
    if (best === null || Math.abs(move) > Math.abs(best.movePct)) {
      best = {
        ...
        preMovePct: Number(pre.toFixed(2)),
      };
    }
  }
  return best;
