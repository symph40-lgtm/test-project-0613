...
export const SIGNAL_CONFIG = {
  ...
  moveAlert: {
    ...
    // 코스피200 선물 ±% — 0.7 등간격 (사용자 확정 2026-07-06: 첫 감지선 0.7, 이후도 0.7 단위)
    futLevels: [0.7, 1.4, 2.1, 2.8, 3.5, 4.2],
    // 장중 반전(스윙) 알림 — 당일 고점 대비 반락폭 / 저점 대비 반등폭(%p) 단계 (사용자 요청 2026-07-06).
    // 절대 등락률로는 안 보이는 반전을 포착: 예) +1.5%→-1.1%는 등락률 -1.1%지만 고점 대비 -2.6%p 반락.
    stockSwingLevels: [4, 7, 10],
    // 선물 반락·반등 경고도 0.7%p 등간격 (사용자 확정 — 고점 +1.5%면 +0.8%에서 1차 경고)
    futSwingLevels: [0.7, 1.4, 2.1, 2.8, 3.5, 4.2],
    // 반전으로 인정할 반대편 극값 최소치(%) — 고점이 이보다 낮으면 단순 일방향 하락(절대 단계가 커버)
    swingMinExtreme: 0.3,
  },

  // ── RV1 하닉 분봉 반전 판정 (사용자 지정 2026-07-07 — 조건 성립 시 즉시 진입신호 문자)
  // 하닉이 5분봉 추세(상승/하락) 중일 때, 아래 중 하나라도 반대 방향으로 성립하면
  // 상승 반전=레버리지 / 하락 반전=인버스 판정. 임계값 %p (전일 종가 대비 등락률 차).
  reversal: {
    m1Single: 0.8,  // 1) 1분봉 1개 종가 반전 ≥ 0.8%
    m1Sum3: 1.0,    // 2) 1분봉 3개 합 ≥ 1.0%
    m5Single: 1.0,  // 3) 5분봉 1개 ≥ 1.0%
