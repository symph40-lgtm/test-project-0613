...
export const SIGNAL_CONFIG = {
  ...
  // X1 갭상승 시초 추격 금지 — 변동성 연동 (사용자 개정 2026-07-20: ATR14가 11.8%인 장에서
  // 고정 2%는 220일 중 29%가 걸리는 과도한 임계로 판명). 실효 임계 = max(gapBigPct, ratio×ATR14).
  gapBigPct: 3,            // 바닥값 (평온장 최소 기준, 기존 2 → 3)
  gapBigAtrRatio: 0.5,     // ATR 연동 계수 — 220일 실측: 갭 0.5~0.8×ATR 구간이 추격 성적 최악(-0.64%)
  // LM 매크로 게이트 (사용자 개정 2026-07-16) — 상대 변화 대신 절대 위험 레벨.
  // "시장이 감내할 수 있는 기준값이 계속 변한다" → 2Y ≥4.2% · 10Y ≥4.6% · 환율 ≥1,540원
  // 중 하나라도 넘으면 레버리지 진입 금지 (인버스는 차단 안 함 — LM은 롱 셋업 전용)
  macroGate: { us2yDanger: 4.2, us10yDanger: 4.6, usdkrwDanger: 1540 },
  ...
  trend: {
    ...
    // 2026-07-15 사용자 지정: T4(외인 선물) 3→4 — 실측(7/14~15 이틀): 추세일 동행 상관 0.71로
    // 최고 수준 (왕복장 -0.15 — 추세 '확인' 지표로서의 가중. 예측 적중률은 54%라 선행 지표는 아님)
    weights: { T1: 3, T2: 2, T3: 1, T4: 4, T5: 1, T6: 2, T7: 2, T8: 2 } as Record<string, number>,
    fullMax: 17,
    ...
    // t4MinDelta30 실측 보정 (2026-07-15): |Δ30분| 분포 중앙 ~1,300억·75% 1,836~3,062억 (7/14~15)
    // — 기존 500은 25% 분위라 사실상 항상 충족 (신호가 아니라 상수). '평소보다 뚜렷한 쏠림'만
    // 방향으로 인정하도록 75% 분위 하단으로 상향. 표본 축적 후 flowCalib 분위 자동화 예정.
    t4MinDelta30: 1500,    // 외인 선물: 30분 ±1,500억 이상이면 방향 인정 (500 → 1500)
  ...
  // ── 스코어 컷 (마스터 4.1·4.2)
  // 2026-07-15 사용자 지정 재설계: "진짜 필수(절대 조건)만 필수로, 나머지는 배점 차등 가점으로,
  // 점수 레인지 확대". 필수는 LM·FG·L3·L4(롱) / S3·S4·FG(숏)만 — 방향·시간대·차단형 게이트.
  //  - L5(외인 수급)를 필수에서 가점(+3)으로: 실측 — 네이버 잠정치가 장중 미제공이라 가동 이래
  //    항상 null → 필수로 묶여 레버리지 확정이 구조적으로 불가능했음 (2026-07-15 발견).
  //    외인이 팔아도 상방인 날도 있음 — 절대 조건이 아니라 확신도 가점이 맞다.
  //  - S1(과열)·S2(매크로 악화)도 가점으로: 근거를 높이는 조건이지 "이것 아니면 안 되는" 조건 아님.
  //  - L1(Bias)·L2(금리환율)는 참고(0점)에서 저배점 가점으로 승격 (사용자 지정: 가점 반영).
  //  - L11(기대인플레)은 소스가 없어 영구 null — 목록에서 제거.
  score: {
    longBonusMax: 14,      // L5 3 + L6 3 + L1 2 + L7 2 + L10 2 + L2 1 + L9 1
    longCandidate: 6,      // 진입 후보 (1/3 비중)
    longStrong: 10,        // 강한 신호
    shortBonusMax: 8,      // S2 3 + S1 2 + S6 2 + S7 1
    shortCandidate: 4,
  },
  ...
  // 2026-07-15 사용자 지정: 개별 종목·선물 급변/스윙 "문자"는 전면 중단 (smsEnabled false) —
  // 장중브리핑·판정 문자가 흐름을 대체. 감지 로직·대시보드 표시는 유지 (한 줄로 재활성 가능).
  moveAlert: {
    smsEnabled: false,
    // 하닉·삼전 ±% (전일 종가 대비 절대 등락률) — 2026-07-13 사용자 지정: FKS200(선물) 문자가
    // 이미 오므로 개별 종목 문자는 50% 축소 (3/5/7/10 4단계 → 간격 2배 4/8/12 3단계.
    // 12 연장은 당일 실측 하닉 -16% — 기존 상한 10 소진)
    stockLevels: [4, 8, 12],
    ...
    // 하닉·삼전 스윙: 1.5%p (사용자 확정 원값 복원 2026-07-14 — "발송량 축소는 추세 판정과 중복되는
    // 문자(급변 단계)만. 별도 지정했던 산/골 반락·반등 감지는 그대로 유지". 7/13의 3.0 축소를 되돌림)
    stockSwingStep: 1.5,
  ...
  flowAlert: {
    smsEnabled: false, // 2026-07-15 사용자 지정: 외인·프로그램 수급 개별 문자 중단 (브리핑이 수급 라인 포함)
    frgnStep: 800,    // 외인 코스피 현물: 극값 대비 800억 반전마다 (폴백)
    prgmStep: 800,    // 프로그램 (차익+비차익): 800억 (폴백)
    minSpan: 300,     // 반전으로 인정할 선행 되돌림 최소치 (억원) — 미세 등락 오탐 방지
    // 반전의 기준이 되는 극값 자체의 최소 규모 (사용자 지정 2026-07-15: "저점 -125억比 +4,076억
    // 반전" 오탐 — 사실상 0 근처 저점은 반전 기준이 아니라 그냥 당일 순매수 확대. 반등은 저점이
    // -minExtreme 이하일 때만, 반락은 고점이 +minExtreme 이상일 때만 인정)
    minExtreme: 500,
  ...
  volumeAlert: {
    smsEnabled: false, // 2026-07-15 사용자 지정: 개별 문자 중단 — 거래량 급증/급감은 장중브리핑 본문에 포함
    ratio: 1.3,
    lowRatio: 0.6,     // 이 이하면 '급감' 표기 (브리핑용)
  ...
  reversal: {
    ...
    // 2026-07-15 사용자 지정: 분봉 조건 문자는 '강한 신호'만 — 변동폭이 조건 임계값의
    // strongRatio배 이상일 때만 발송. 약한 성립은 문자 없이 판정·대시보드에만 반영.
    smsStrongOnly: true,
    strongRatio: 1.5,
  },
