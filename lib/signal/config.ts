...
export const SIGNAL_CONFIG = {
  ...
  // C6 미 10년물 금리 절대 레벨 단계 (사용자 지정 2026-07-13 2차: "단계별로 위험 수위를 높여줘").
  // 단계가 오를수록 하방 판정 조건이 느슨해짐 — 경계: 상승 중일 때만 / 주의: 보합도 /
  // 위험: 뚜렷한 완화 아니면 / 최고위험: 무조건 하방 + 가중 2. 마지막 단계는 상한 없음(4.62↑).
  us10yBands: [
    { from: 4.55, label: "경계" },
    { from: 4.57, label: "주의" },
    { from: 4.59, label: "위험" },
    { from: 4.62, label: "최고위험" },
  ] as { from: number; label: string }[],
  ...
  trend: {
    ...
    // 추세일 확정 보수 게이트 (사용자 지정 2026-07-13: "매크로 방향성이 뚜렷한 날, 분봉에서
    // 변동성보다 추세가 센 날에만 추세일 판정 — 요즘처럼 변동성 심할 땐 보수적으로").
    // T-스코어가 추세일이어도 ①축1(매크로)이 같은 방향·강도 이상으로 정렬 ②DC2(변동성 대비
    // 순추세 효율)가 기준 이상 — 둘 다 충족해야 '추세일' 유지, 아니면 약한추세(1/3 비중)로 강등.
    strongDay: { minBiasStrength: 2, dc2Min: 0.35 },
  ...
  moveAlert: {
    ...
    // 코스피200 선물 ±% — 0.56 등간격 (2026-07-13 사용자 지정: 선물 1% ≈ 주식 2%라 변화폭이 커서
    // 기존 0.7 간격을 20% 축소). 폭락일 대비 12.32까지 연장 (2026-07-13 실측 -9.8 도달 — 기존 상한 소진)
    futLevels: [0.56, 1.12, 1.68, 2.24, 2.8, 3.36, 3.92, 4.48, 5.04, 5.6, 6.16, 6.72, 7.28, 7.84, 8.4, 8.96, 9.52, 10.08, 10.64, 11.2, 11.76, 12.32],
    ...
    futSwingStep: 0.56,   // 선물: 저점(고점) 대비 0.56%p마다 (2026-07-13 사용자 지정 — 20% 축소)
  ...
  reversal: {
    ...
    // 반복(2·3차) 추가 조건 (사용자 피드백 2026-07-13): 쿨다운만으론 같은 반등이 조건만 바뀌어
    // (1분봉5개 ↔ 5분봉 1개 — 같은 5분 구간의 두 잣대) 재감지돼 중복 발송됨. 직전 발송 시점의
    // 하닉 등락률 대비 그 방향으로 이 %p 이상 '추가 진행'했을 때만 반복 발송.
    repeatMinProgressPct: 0.8,
    // 신호 품질 필터 (사용자 피드백 2026-07-13 2차 — 하락 추세일 14시 이후 윗꼬리 반등에
    // 레버리지 3차까지 발송, 15:05 재급락. "레버리지 판단을 하지 말았어야"):
    // 윗꼬리 필터 — 신호 창 극값 대비 이미 되돌린 폭이 기준 이상(이익실현에 흡수된 반등)이면 무효.
    // 되돌림 ≥ max(wickRetraceMinPp, |변동폭| × wickRetraceRatio)면 발송 안 함.
    // (발송 시간대·추세일 정합 게이트는 alerts.ts buildReversalAlert — ①판정 구간만 ②추세일
    // 확정과 반대 방향 차단)
    wickRetraceRatio: 0.4,
    wickRetraceMinPp: 0.3,
  },
