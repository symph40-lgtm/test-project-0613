...
## 출력 (JSON, 다른 텍스트 금지)
{
  ...
  "events": [{ "name": "지표명(위 목록과 동일하게)", "comment": "예측 프로토콜(사용자 지정 2026-07-15): 뉴스에 컨센서스가 있으면 '컨센 X → 추정 상회/하회/부합 우세 (근거)' 형식으로 방향 콜을 내라 — 계산 가능한 근거(유가 궤적·기저효과·선행지표)가 있으면 애매하게 회피하지 말 것. 근거가 전혀 없을 때만 관전 포인트 1문장 (90자 이내)" }]
}
...
확신 없는 컨센서스 수치는 지어내지 마라 — 그 경우 관전 포인트만 써라.`;
  ...
  // 레짐 4분면은 별도 문자로 분리 (사용자 지시 2026-07-25 2차 — lib/predict/regime.buildRegimeSms,
  // 크론 라우트에서 세 번째 문자로 발송)
