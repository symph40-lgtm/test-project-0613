  ...
  return (
    <details className="mt-2 rounded-[10px] border border-hairline bg-pearl/40">
      ...
      <div className="space-y-3 px-3 pb-3 pt-1">
        ...
        <Term t="매수매력 (0~100) — '반등 조짐이 실제로 보이는가' · 50점 분기">
          <b>50점이 분기점</b>입니다. <b>가격이 실제로 돌아서는 신호가 하나라도 있어야 50 이상</b> — 없으면(계속 하락·수급 이탈) 과매도·낙폭과대여도 50 미만(떨어지는 칼).
          <br /><b>반등 조짐(50↑ 게이트)</b>: ① 당일 반등 양봉(+0.4%↑, 강하면 +1.5%&amp;거래량 1.3배↑) ② 정배열 회복 ③ 외인·기관 동시순매수 — 셋 중 하나라도.
          <br /><b>조짐 있으면(50 기준)</b>: 양봉 강도·정배열·수급 유입으로 가산, 과매도 탈출(RSI 30~45)·볼린저 하단탈출·상대강도&gt;0 보조.
          <br /><span className="text-blue-600"><b>조짐 없으면(50 미만)</b>: 당일 하락·수급 동반 이탈·역배열 음봉이 클수록 더 낮게.</span> 과매도는 '여건'일 뿐 조짐 아님.
          <br />→ 철강처럼 <b>오늘까지 계속 빠지는 섹터는 낮게</b>, 막 반등 양봉+수급이 들어오는 섹터만 높게.
        </Term>
        <Term t="주도력 ↔ 매수매력 (반대 개념)">
          <b>주도력↑</b> = 이미 강세(신고가, 예: 반도체) → 추격 부담.
          <b>매수매력↑</b> = 충분히 조정된 뒤 <b>반등이 시작되는 신호</b>(수급 전환·반등 양봉·과매도 탈출)가 잡히는 섹터.
        </Term>
              ...
              {[...sectors].sort((a, b) => b.buyAttract - a.buyAttract).map((s) => {
                ...
                return (
                  <tr key={s.code} className={s.isSemi ? "bg-guard/5" : ""}>
                    ...
                    <td className={`px-2 py-1.5 text-right tabular-nums font-semibold ${s.buyAttract >= 50 ? "text-guard" : "text-blue-600"}`}>{s.buyAttract}</td>
                  </tr>
                );
              })}
            </tbody>
...
매수매력도 순 정렬. <b>주도력</b>=지금 강한 섹터(전고점 위치·상대강도·정배열·모멘텀·수급) / <b>매수매력</b>=반등 조짐(50점 분기 — 당일 반등 양봉·정배열 회복·동시순매수 있어야 50↑, 계속 빠지면 50↓). <b>둘은 다릅니다</b> — 반도체는 주도력↑·매수매력↓(신고가), 조정 섹터는 주도력↓·매수매력↑일 수 있음. 위 AI '후보'는 매수매력도 상위에서 선정됩니다. <b>정배열</b>(현재가&gt;20일선&gt;60일선)=상승추세, <b>역배열</b>=하락추세 — 후보 섹터는 대부분 역배열(조정 중)이라 반등 매력으로 봅니다.
