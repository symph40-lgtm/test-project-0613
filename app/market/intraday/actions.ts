...
import { fetchReflectInsights, formatInsightsForPrompt } from "@/lib/ai/insights";
import {
  calculateRiskScores,
...
export async function getMarketExplain(): Promise<MarketExplain> {
  const [market, news, fut, insights] = await Promise.all([
    fetchMarketData(),
    fetchMarketNews(6),
    fetchKospi200Futures(),
    fetchReflectInsights(4),
  ]);
...
## 환율 표기 규칙 (반드시 준수)
- 위 환율 값은 '원/달러 환율(USDKRW=X)'이며 달러인덱스(DXY)가 아닙니다. '달러인덱스'나 '달러 가치 전반'으로 해석하지 마십시오.
- 환율 언급 시 항상 '원/달러 환율'로 부르고, 하락(마이너스)이면 '원/달러 환율 하락(원화 강세)', 상승(플러스)이면 '원/달러 환율 상승(원화 약세)'로 표기하십시오. '달러화 약세/강세' 같은 모호한 표현은 쓰지 마십시오.
...
export async function getIntradayConsult(): Promise<IntradayConsult> {
  ...
  const [market, fut, insights] = await Promise.all([
    fetchMarketData(),
    fetchKospi200Futures(),
    fetchReflectInsights(4),
  ]);
