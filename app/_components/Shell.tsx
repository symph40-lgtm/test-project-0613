...
export function GlobalNav() {
  return (
    <header className="sticky top-0 z-30 flex h-11 items-center justify-between bg-black px-4 text-white sm:px-6">
      <Link href="/" className="flex items-center gap-1.5 text-[14px] md:text-[12px] tracking-[-0.12px]">
  ...
  return (
    <div className="sticky top-11 z-20 flex h-[52px] items-center justify-between border-b border-hairline bg-parchment/80 px-4 backdrop-blur-md backdrop-saturate-150 sm:px-6">
      <div className="flex items-center gap-2">
        <BackButton />
        <h1 className="text-[24px] md:text-[21px] font-semibold tracking-[0.231px]">{title}</h1>
        {badge ? (
          <span className="rounded-full bg-ink px-2 py-0.5 text-[13px] md:text-[11px] font-normal text-white">
            {badge}
          </span>
        ) : null}
      </div>
...
export function Disclaimer() {
  return (
    <p className="mt-8 text-[15px] md:text-[12px] leading-relaxed text-ink-48">
