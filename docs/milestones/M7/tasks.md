...
- [x] T14. 판정 구간 09:30~13:30 연장 + 장중 재형성(지연) 추세 감지 — 최근 90분 롤링 창(DC1·순이동·전환)으로 초반 횡보일도 재평가, T1은 "레인지 밖 유지 시간" 기준으로 개선. 지연 추세는 약한추세(1/3 비중)부터
- [x] T15. AI 자동 정성 주석 — `lib/signal/autoAnnotate.ts` (매일 뉴스+매크로를 Claude haiku로 분석해 원인 태그·주석·L7·L8 자동 채움, 하루 1회·실패 시 15분 재시도, 사용자 입력이 항상 우선. 016 마이그레이션)
